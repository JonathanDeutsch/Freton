function varargout = kat_gamma(varargin)
% KAT_GAMMA MATLAB code for kat_gamma.fig
%      KAT_GAMMA, by itself, creates a new KAT_GAMMA or raises the existing
%      singleton*.
%
...
% Last Modified by GUIDE v2.5 02-Jun-2017 15:02:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @kat_gamma_OpeningFcn, ...
                   'gui_OutputFcn',  @kat_gamma_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before kat_gamma is made visible.
function kat_gamma_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = kat_gamma_OutputFcn(hObject, eventdata, handles) 
...
save('gamma_to_analyse.mat','time_axis','don_tot_trace','don_bckg_trace','don_spec_trace',...
                 'accep_tot_trace','accep_bckg_trace','accep_spec_trace','Mode1');
...
fret_pre = accep_pre/(accep_pre + don_pre);
all_gamma_t = gamma;
all_fret_t = fret_pre;
...
set(handles.edit9, 'String', num2str(fret_pre));
...
%% beta and gamma corrections
% 
% numer_fret1 = accep_sel - (beta.*don_sel);
% denom_fret1 = numer_fret1 + (gamma.*don_sel);
% fret_calc_trace1 = numer_fret1./denom_fret1;
% 
% axes(handles.axes4);
% plot(time_axis, fret_calc_trace1,'m');
% ylabel('FRET 2 (\beta - \gamma)')
% ylim([0 1]);
...
% --- Executes on button press in get_fig.
function get_fig_Callback(hObject, eventdata, handles)
% hObject    handle to get_fig (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global all_gamma
global all_fret
